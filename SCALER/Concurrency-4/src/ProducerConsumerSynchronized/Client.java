...
public class Client {
    public static void main(String[] args){
        ...
        * */
        ...
        /* Here we solved the inconsistency problem by making store synchronized, but there
        is newer kind of inconsistency in terms of utilizing total number of producers and
        consumers like right now only p1 is producing all the six shirts and then c1 is buying
        all the shirts, so we are not utilizing our resources well.
        So if our store have some functionality, like if some producer produced a shirt, then it
        will notify the consumer, similarly if a shirt is consumed then consumer will notify the
        producer to produce a new shirt.

        We should allow multiple producers and consumers in store in a controlled way.
        So again, allow as many producers as many empty slots are there and as many consumers
        as many filled slots are there.

        Here comes the Semaphores to solve this issue.
